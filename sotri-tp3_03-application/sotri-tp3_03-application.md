...
## 3. Conclusiones
El diseño de firmware cumple exitosamente con los requisitos lógicos de concurrencia y control de capacidad. La combinación de **Semáforos Binarios** (para la sincronización asincrónica de las tareas con los estímulos inyectados por la tarea de prueba `task_test`) junto con un **Mutex** (para proteger la variable global compartida) demuestra ser el estándar óptimo y eficiente en FreeRTOS. Esta arquitectura permite coordinar el acceso concurrente a un recurso de capacidad limitada (emulando la zona física del cruce) sin sufrir interbloqueos (*deadlocks*) ni corrupción de datos, validando la robustez del sistema antes de su integración con sensores reales.