...
## 3. Conclusiones
El diseño de firmware demuestra ser robusto frente a condiciones de carrera (*Race Conditions*). El uso del Mutex resuelve nativamente el problema lógico de contención del recurso (emulando el espacio confinado de la esclusa), garantizando la exclusión mutua a nivel de software. Por otro lado, el uso emparejado de Semáforos Binarios para recibir las solicitudes de apertura y las confirmaciones de cierre asegura que la tarea de la puerta libere la zona compartida solo cuando el proceso (simulado por la tarea inyectora de estímulos `task_test`) ha culminado por completo. De esta manera, la lógica del sistema operativo cumple estrictamente con la especificación de "una sola puerta abierta a la vez", dejando el código listo para su eventual integración con actuadores y sensores de hardware reales.