...
La implementación emulada en el microcontrolador STM32 cumple rigurosamente con los patrones de sincronización descritos en el modelo teórico de Allen B. Downey (*The Little Book of Semaphores*). El uso de un Mutex con propiedad de hilo (*thread ownership*) y herencia de prioridad permite proteger la sección crítica que representa el estado de ocupación de la esclusa, garantizando la exclusión mutua estricta a nivel de software. Complementariamente, el uso de múltiples Semáforos Binarios para la señalización asincrónica de eventos (coordinando las solicitudes de apertura y confirmaciones de cierre inyectadas por la tarea de prueba `task_test`) elimina las condiciones de carrera y los estados lógicos corruptos. La salida obtenida en la terminal serial valida con éxito la robustez del diseño de firmware antes de su eventual portabilidad hacia un entorno con actuadores físicos reales.